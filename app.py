...
DATA_FILE = "finland1.csv"  # Updated to the correct file name
data = load_data(DATA_FILE)
tickers = data["Ticker"].unique()

ticker = st.sidebar.selectbox("Select Ticker:", options=tickers)
today = dt.datetime.today().date()  # Ensure it's a date object

# Adjust the start_date and end_date to ensure they make sense
min_date = data["Date"].min().date()  # Min date from the dataset
max_date = data["Date"].max().date()  # Max date from the dataset
...
end_date = st.sidebar.date_input(
    "End Date:",
    today,
    min_value=start_date,  # Ensure end_date is greater than or equal to start_date
    max_value=max_date,  # Ensure the max date from dataset
)
